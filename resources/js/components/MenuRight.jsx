...
import { DataMenuRight } from "../../data/DataMenuRight";
...
export const MenuRight = () => {
    ...
    return (
        <div className="relative flex w-fit h-fit gap-2">
            {TABS.map((t, index) => (
                <Tab
                    key={t.id}
                    selected={selected}
            ...
            <div className="relative hidden xl:flex items-center gap-5 text-white ">
                <input
                    type="text"
                    placeholder="Cari..."
                    value={search}
...
const Tab = ({
    ...
    return (
        ...
        >
            ...
            <button
                id={`shift-tab-${tab}`}
                className={`hidden xl:flex items-center gap-1 rounded-full w-fit h-fit px-3 py-1.5 text-sm transition-colors  cursor-pointer group  ${
                ...
                {menu.submenu?.length > 0 &&
                    menu.submenu.map((item, idx) => (
                        <a
                            key={idx}
                            href={item.href}
                            className="block text-sm text-theme hover:text-lime-600"
                        >
                            {item.label}
                        </a>
                    ))}
...
export const TABS = DataMenuRight.map((menu, idx) => ({
    id: idx + 1,
    title: menu.label,
